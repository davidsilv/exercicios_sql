drop table if exists tb_avg_compra_venda;
create table tb_avg_compra_venda as
...
tb_tempo_vendas_seller as (
        select *,
                julianday(data_venda) - julianday(lag_data_venda) as diff_days
...
select seller_id,
        avg(diff_days) as tempo_medio_em_dias
